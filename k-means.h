...
DataFrame k_means(const DataFrame& data, size_t k, size_t number_of_iterations)
{
    ...
    for (size_t iteration = 0; iteration < number_of_iterations; ++iteration)
    {
        ...
        DataFrame newMeans(k);
        std::vector<size_t> counts(k, 0);
        for (size_t point = 0; point < data.size(); ++point)
        {
            const auto cluster = assignments[point];
            newMeans[cluster].x += data[point].x;
            newMeans[cluster].y += data[point].y;
            counts[cluster] += 1;
        }
        ...
        for (size_t cluster = 0; cluster < k; ++cluster)
        {
            ...
            means[cluster].x = newMeans[cluster].x / count;
            means[cluster].y = newMeans[cluster].y / count;
        }
    }
